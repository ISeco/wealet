...
- **Moneda extranjera (USD/EUR → CLP)**: una transacción puede ingresarse en moneda extranjera indicando el tipo de cambio. Se guarda convertida a la moneda base del usuario (`users.base_currency`, default `CLP`) en `amount` — la fuente de verdad contable — y conserva `original_amount`/`original_currency`/`exchange_rate` como metadato. El backend recalcula el `amount` server-side (`convertToBase`, no confía en el monto del cliente); la agregación de balances no cambia porque todo lo sumado está en la base. `TransactionFormModal` ofrece selector de moneda + campo de tipo de cambio con prellenado opcional desde `GET /exchange-rate` (proxy backend a mindicador.cl — best-effort: timeout ~3s + cache en memoria ~6h + fallback a `null`; el valor manual siempre manda). La tabla muestra una nota de procedencia (`formatForeignNote`, ej. `US$9,99 · TC 948,95`) cuando la transacción tiene los campos original, propagados por `/activity`. Ver ADR-09.
